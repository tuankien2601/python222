...
represents a full font name, such as \code{u'LatinBold19'}, or as a plain 
...
\begin{methoddesc}{text}{coordseq, text\optional{fill=0, font=u"LatinBold12''}}
...
Copies the source area from the given \var{image} to the target area in 
this drawable. The source area is copied in its entirety if \var{mask} is 
not given or is set to \code{None}. If the mask is given, the source area 
is copied where the mask is white. \var{mask} can be either \code{None} 
or a 1-bit (black and white) \class{Image} and must be of the same size as 
the source image.
