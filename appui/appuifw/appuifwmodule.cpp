...
multi_selection_list(PyObject* /*self*/, PyObject *args, PyObject *kwds)
{
...
#else
    TRAP(error, {
    icons->AppendL(env->CreateIconL(KIconsFile, EMbmAvkonQgn_prop_checkbox_on, EMbmAvkonQgn_prop_checkbox_on_mask));
    icons->AppendL(env->CreateIconL(KIconsFile, EMbmAvkonQgn_prop_checkbox_off, EMbmAvkonQgn_prop_checkbox_off_mask));
    });
  } else 
    TRAP(error, 
	 icons->AppendL(env->CreateIconL(KIconsFile, EMbmAvkonQgn_indi_marked_add, EMbmAvkonQgn_indi_marked_add_mask)));
#endif /* SERIES60_VERSION */
  ...
  if (error == KErrNone) {
    ...
    for (int i=0; i < selItemNum; i++) 
      error = PyTuple_SetItem(selected, i, Py_BuildValue("i", selected_items->operator[](i)) );
				     ...
				     CArrayPtr<CGulIcon>* icons = NULL)
{
  ...
  for (int i = 0; i < sz; i++) {
      ...
      if (!PyTuple_Check(t))
        ...
        if ((!PyUnicode_Check(s1)) || (!PyUnicode_Check(s2)))
          ...
          if (is_popup_style)
            ...
            temp.Copy(KSeparatorTab);
          temp.Append(PyUnicode_AsUnicode(s1),
                      Min(PyUnicode_GetSize(s1), KMaxFileName));
          temp.Append(KSeparatorTab);
          temp.Append(PyUnicode_AsUnicode(s2),
	...
	if (!PyArg_ParseTuple(t, "OO!", &s1, &Icon_type, &io))
	  ...
	  temp.Append(PyUnicode_AsUnicode(s1), Min(PyUnicode_GetSize(s1), KMaxFileName));
        }
...
public:
  ...
  TRgb TextColor() { return iTextColor; };
  void SetHighlightColor(TRgb color);
  TRgb HighlightColor() { return iHLColor; };
...
private:
  ...
  CCoeEnv* 	iCoeEnv;
  CFont*	aFont;
  TFontSpec*	aFontSpec;	//dont'use iFontSpec: is confusing with base class members
};
...
{
  if (iCoeEnv && aFont) {
    iCoeEnv->ReleaseScreenFont(aFont);
    iCoeEnv = NULL;
    aFont = NULL;
  }
...
void CAppuifwText::SetHighlightColor(TRgb color)
{
  ...
  iHLColor=color;
  iCharFormat.iFontPresentation.iHighlightColor = iHLColor;
}

void CAppuifwText::SetFont(TDesC& fontName)
{
  ...
  aFontSpec = new TFontSpec(fontName, 0); 
  ...
  iCharFormat.iFontSpec.iTypeface = aFontSpec->iTypeface;
}

const CFont* CAppuifwText::SystemFont(TPtrC8& font_type)
{
  const CFont* systemFont=NULL;

  if (font_type.Compare(KNormal) == 0)
    systemFont = (CEikonEnv::Static())->NormalFont();
  else if (font_type.Compare(KAnnotation) == 0)
    systemFont = (CEikonEnv::Static())->AnnotationFont();
  else if (font_type.Compare(KTitle) == 0)
    systemFont = (CEikonEnv::Static())->TitleFont();
  else if (font_type.Compare(KLegend) == 0)
    systemFont = (CEikonEnv::Static())->LegendFont();
  else if (font_type.Compare(KSymbol) == 0)
    systemFont = (CEikonEnv::Static())->SymbolFont();
  else if (font_type.Compare(KDense) == 0)
    systemFont = (CEikonEnv::Static())->DenseFont();
...
extern "C" {
  ...
  Text_getattr(Text_object *op, char *name)
  {
	...
	return Py_False;
      ...
      TTypeface fontFace = (op->ob_control->Font()).iTypeface;   
      return Py_BuildValue("u#", fontFace.iName.Ptr(), fontFace.iName.Length());
    }
  ...
  Text_setattr(Text_object *op, char *name, PyObject *v)
  {
	...
	isUnicode = false;
        ...
        PyErr_SetString(PyExc_TypeError, "font must be a unicode string or valid string label");
        return -1;
      }
      
      if (!isUnicode) {
	char* name = PyString_AsString(v);
	int name_len = PyString_Size(v);

	TPtrC8 font_type((TUint8*)name, name_len);

	const CFont* aFont = op->ob_control->SystemFont(font_type);
	if (!aFont) {
	  PyErr_SetString(PyExc_ValueError, "invalid font label");
	  return -1;
	}

	TBuf<KMaxTypefaceNameLength> buff = (aFont->FontSpecInTwips()).iTypeface.iName;

	op->ob_control->SetFont(buff);
      } else {
	const char* name = PyUnicode_AS_DATA(v);
	int name_len = PyUnicode_GetSize(v);

	TPtrC buf_value((TUint16*)name, name_len);

	op->ob_control->SetFont(buf_value);
      }
...
PyObject* CPyFormFields::ItemValidate(PyObject* t)
{
  ...
  case EEikCtNumberEditor:
    // XXX giving e.g. 42L fails (should work), check this situation also
    if (!PyInt_Check(val)) { 
      PyErr_SetString(PyExc_TypeError,
                      "Form number field, integer value expected");
      return NULL;
    }
    break;
  case EEikCtFlPtEd:
