...
const TUint KObRfcommChannel = 1;
...
class TSdpAttributeParser : public MSdpAttributeValueVisitor 
{
 public:
    ...
    TSdpAttributeParser::TSdpAttributeParser(TSdpAttributeList& aNodeList, MSdpAttributeNotifier& aObserver):
...
const TSdpAttributeParser::SSdpAttributeNode gObexProtocolListData[] = 
{
        { TSdpAttributeParser::ECheckType, ETypeDES },
            ...
            { TSdpAttributeParser::ECheckType, ETypeDES },
                { TSdpAttributeParser::ECheckValue, ETypeUUID, KRFCOMM },
                { TSdpAttributeParser::EReadValue, ETypeUint, KObRfcommChannel },
            { TSdpAttributeParser::ECheckEnd }, 
            { TSdpAttributeParser::ECheckType, ETypeDES },
...
{ 
  if (aServiceType == RfcommService)
    aKey = static_cast<TInt>(KRfcommChannel);
  else
    aKey = static_cast<TInt>(KObRfcommChannel);
  iPort = aValue.Uint(); 
}
...
class CSocketEngine : public CBase
{
 ...
 public:
  CSocketEngine():iAdvertiser(NULL),iPort(-1) {;}
...
new_Socket_object(PyObject* /*self*/, PyObject *args)
{
  ...
  if (family == KAfInet) {
    if (type == KSockStream) {
      ...
      if (protocol && (protocol != KProtocolInetUdp))
        return PySocket_Err("Bad protocol");
      else 
    ...
    if (type == KSockStream) {
      if ((protocol == KRFCOMM) || (protocol == 0))
        protocol = KRFCOMM;
      else 
        return PySocket_Err("Bad protocol");
    }
...
get_host_by_name(PyObject* /*self*/, PyObject *args)
{
  ...
  PyObject* str16 = PyUnicode_FromObject(Py_BuildValue("s#", ipText, ipTextL));
