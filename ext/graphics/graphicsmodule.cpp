...
struct CFsContainer : public CBase {
  CFsContainer() {}
  void ConstructL() {
...
static TInt app_callback_handler(void* func);
...
graphics_GetRFs()
{
  return ((CFsContainer *)SPyGetGlobalString("GraphicsRFs"))->iFs;
}
...
private:
  void InvokeCompletionCallback() {
    if (iCompletionCallback) {
      PyObject *arg=Py_BuildValue("(i)",iStatus);
...
Draw_blit(Draw_object *self, PyObject *args, PyObject *keywds)
{
  ...
  if (mask_object) {
    ...
    if (mask_bitmap->DisplayMode() != EGray2) {
      PyErr_SetString(PyExc_TypeError, "Mask must be a binary (1-bit) Image");
      return NULL;
    }
  }
...
{
  int cx=(bbox.iBr.iX-bbox.iTl.iX)/2;
  int cy=(bbox.iBr.iY-bbox.iTl.iY)/2;
  return TPoint(cx+cos(angle)*1024, cy-sin(angle)*1024);
}
...
#define DEFTYPE(name,type_template)  do {				\
  ...
  static void
  _graphics_fscontainer_dealloc(void *container)    
  {
    delete (CFsContainer *)container;
  }  
...
#define DEFLONGX(name) DEFLONG(#name,name)  
    ...
    DEFLONGX(ROTATE_180);
    DEFLONGX(ROTATE_270);    
    //CFsContainer *fsContainer=new CFsContainer;    
    //fsContainer->ConstructL();
    //PyObject *rfsobj=PyCObject_FromVoidPtr(fsContainer,_graphics_fscontainer_dealloc);
    //SPyAddGlobalString("GraphicsRFs", rfsobj);
    // We want the global dictionary to be the only thing holding
    // GraphicsRFs alive.
    //Py_DECREF(rfsobj);
  ...
  DL_EXPORT(void) zfinalizegraphics(void)
  {
    //RFbsSession::Disconnect();
#ifdef ICL_SUPPORT
    //SPyRemoveGlobalString("GraphicsRFs");
#endif //ICL_SUPPORT
  }
} /* extern "C" */
