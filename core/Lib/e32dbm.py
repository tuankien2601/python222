...
#
# Note:
# 1) The internal representation of keys and values is unicode,
#    although on input also strings are accepted. This may lead
#    to situations where d[key] = value; if value == d[key]: failing
#    If in doubt, use only unicode keys and values.
# 2) There is currently no support for concurrent access.
# 3) The internal database format may still change. No guarantee
